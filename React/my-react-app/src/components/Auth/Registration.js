  ...
  if (globals.value.accessToken == null)
    return(
      <div className="mt-5 pt-5 pb-5 w-100 border rounded-3 bg-info bg-gradient d-flex align-items-center flex-column">
        ...
        <button id="confirm" className="w-25 mt-2" onClick={dispatch.bind(null,{type: 'USER_REGISTER_REQUESTED', payload: {state, dispatch, setState:setStateGlobal}})}>Register</button>
      </div>
    )
  else {
    history.push({
