...
import { changePublished, changeTrash, deletePhoto } from '../Utils/editPhotoFuntions';
    ...
    return(
      <div className="d-flex flex-column" margin-bottom="1000">
        ...
        <div className="pb-3">
          ...
          <button className="ms-3 rounded-3" onClick={changePublished.bind(null, token, loading, `https://localhost:44340/api/Admin/EditPhoto`, `https://localhost:44340/api/Home/${id}`, dispatch, setState)}>Change</button>
        </div>
        <div>
          <span className="pb-3 text-dark">Trash: {loading.value.data.isTrash ? "Yes" : "No"}</span>
          <button className="ms-3 rounded-3" onClick={changeTrash.bind(null, token, loading, `https://localhost:44340/api/Admin/EditPhoto`, `https://localhost:44340/api/Home/${id}`, dispatch, setState)}>Change</button>
        </div>
        <button className="w-25 ms-5 mt-5 bg-danger rounded-3" onClick={deletePhoto.bind(null, token, `https://localhost:44340/api/Admin/DeletePhoto/${id}`, history, '/admin/photo')}>Delete</button>
      </div>
    );
